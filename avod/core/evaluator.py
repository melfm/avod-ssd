...
class Evaluator:
    ...
    def __init__(self,
        ...
        self.rpn_model = isinstance(self.model, RpnModel)
        self.avod_model = isinstance(self.model, AvodModel)
        self.ssd_model = isinstance(self.model, AvodSSDModel)
    ...
    def run_checkpoint_once(self, checkpoint_to_restore):
        ...
        if validation and (self.rpn_model or self.avod_model):
            eval_rpn_losses = self._create_rpn_losses_dict()

        if self.rpn_model or self.avod_model:
        ...
        if self.avod_model or self.ssd_model:
            # Make sure the box representation is valid
            model_type = self.model_config.model_type
            if self.avod_model:
                box_rep = \
                    model_type.avod.avod_config.avod_box_representation
            elif self.ssd_model:
                box_rep = \
                    model_type.avod_ssd.avod_config.avod_box_representation
        ...
        while current_epoch == self.model.dataset.epochs_completed:
            ...
            if validation:
                ...
                if self.rpn_model or self.avod_model:
                    rpn_objectness_loss = \
                        eval_losses[RpnModel.LOSS_RPN_OBJECTNESS]
                    rpn_regression_loss = \
                        eval_losses[RpnModel.LOSS_RPN_REGRESSION]
                    ...
                    # File paths for saving proposals
                    rpn_file_path = prop_score_predictions_dir + \
                        "/{}.txt".format(sample_name)
                    # Save proposals
        ...
        if validation:
            if self.avod_model or self.rpn_model:
        ...
        print("Step {}: Finished evaluation".format(global_step))
    ...
    def _update_avod_box_cls_loc_orient_losses(self,
        ...
        if self.avod_model:
            ...
            avod_classification_loss = \
                eval_losses[AvodSSDModel.LOSS_FINAL_CLASSIFICATION]
            avod_regression_loss = \
                eval_losses[AvodSSDModel.LOSS_FINAL_REGRESSION]

            avod_localization_loss = \
                eval_losses[AvodSSDModel.LOSS_FINAL_LOCALIZATION]
            avod_orientation_loss = \
                eval_losses[AvodSSDModel.LOSS_FINAL_ORIENTATION]
    ...
    def get_evaluated_ckpts(self):
        ...
        if self.model_name == 'avod_model' or \
                self.model_name == 'avod_ssd_model':
            avg_loss_file_path = predictions_base_dir + '/avod_avg_losses.csv'
        else:
            avg_loss_file_path = predictions_base_dir + '/rpn_avg_losses.csv'
    ...
    def get_cls_accuracy(self,
                         predictions,
                         eval_avod_losses=None,
                         eval_rpn_losses=None,
                         global_step=0):
        ...
        if self.avod_model:
            ...
            classification_pred = \
                predictions[AvodSSDModel.PRED_MB_CLASSIFICATION_SOFTMAX]
            classification_gt = \
                predictions[AvodSSDModel.PRED_MB_CLASSIFICATIONS_GT]
            classification_accuracy = self.calculate_cls_accuracy(
                classification_pred, classification_gt)
            ...
            print("Step {}: AVODSSD Classification Accuracy: {}".format(
                global_step, classification_accuracy))

    def calculate_cls_accuracy(self, cls_pred, cls_gt):
        """Calculates accuracy of predicted objectness/classification wrt
        to the labels.
    ...
    def get_avod_predicted_boxes_3d_and_scores(self, predictions,
        ...
        if box_rep == 'box_3d':
            ...
            if self.avod_model:
                ...
                final_pred_anchors = predictions[
                    AvodSSDModel.PRED_TOP_PREDICTION_ANCHORS]
                final_pred_orientations = predictions[
                    AvodSSDModel.PRED_TOP_ORIENTATIONS]
            ...
            if self.avod_model:
                ...
                final_pred_boxes_3d = predictions[
                    AvodSSDModel.PRED_TOP_PREDICTION_BOXES_3D]
        ...
        if self.avod_model:
            ...
            final_pred_softmax = predictions[
                AvodSSDModel.PRED_TOP_CLASSIFICATION_SOFTMAX]
        ...
        if box_rep in ['box_8c', 'box_8co']:
            if self.avod_model:
                final_pred_box_corners = \
                    predictions[AvodModel.PRED_TOP_BOXES_8C]
            elif self.ssd_model:
                final_pred_box_corners = \
                    predictions[AvodSSDModel.PRED_TOP_BOXES_8C]

        elif box_rep in ['box_4c', 'box_4ca']:
            if self.avod_model:
                final_pred_box_corners = \
                    predictions[AvodModel.PRED_TOP_BOXES_4C]
            elif self.ssd_model:
                final_pred_box_corners = \
                    predictions[AvodSSDModel.PRED_TOP_BOXES_4C]
